...
int main(int argc, char** argv) {
  ...
  swapchain_create_info.setPreTransform(surface_capabilities.currentTransform);
  ...
  std::vector<char> vert_shader_code = slurp_file("vert.spv");
  ...
  std::vector<char> frag_shader_code = slurp_file("frag.spv");
  ...
  descriptor_set_layout_binding.setStageFlags(vk::ShaderStageFlagBits::eVertex);
  ...
  for (size_t i = 0; i < swapchain_framebuffers.size(); i++) {
    ...
    command_buffer.draw(vertices.size(), 1 /*instanceCount*/, 0 /*firstVertex*/,
                        0 /*firstInstance*/);
