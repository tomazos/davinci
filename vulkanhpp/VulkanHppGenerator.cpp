...
#define DUMP_EXPR(os, expr) os << #expr << ": " << (expr) << std::endl
...
void VulkanHppGenerator::writeTypeEnum(std::ostream & os, EnumData const& enumData)
{
  ...
  enterProtect(os, enumData.protect);
  os << "ENUM_NAME " << enumData.name << std::endl;
  for (size_t i = 0; i<enumData.values.size(); i++)
  {
    os << "enumData.values[i].name: " << enumData.values[i].name << std::endl;
    os << "enumData.values[i].value: " << enumData.values[i].value << std::endl;
    os << "enumData.values[i].alias: " << enumData.values[i].alias << std::endl;
  }
...
void VulkanHppGenerator::writeDelegationClassStatic(std::ostream &os)
{
  for (const auto& command : m_commands)
  {
    ...
    DUMP_EXPR(os, command.second.unchangedReturnType);
    DUMP_EXPR(os, command.second.fullName);
    for (auto param : command.second.params)
    {
      DUMP_EXPR(os, param.unchangedType);
      DUMP_EXPR(os, param.name);
      DUMP_EXPR(os, param.arraySize);
    }
...
int main( int argc, char **argv )
{
  try {
    ...
    if (argc != 3) {
      std::cerr << "USAGE: " << argv[0] << " <in.xml> <out.data>" << std::endl;
      return EXIT_FAILURE;
    }

    std::string filename = argv[1];
    std::cout << "Loading vk.xml from " << filename << std::endl;
    std::cout << "Writing vulkan.hpp to " << argv[2] << std::endl;
