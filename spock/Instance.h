...
#include "spock/Utils.h"
#include "spock/Version.h"
...
class Instance {
 public:
  ...
  Instance(const std::string& application_name,
    ...
    if (physical_devices.empty())
    ...
    else if (FLAGS_vkdevice < 0 || FLAGS_vkdevice > int32_t(physical_devices.size())) {
      ...
      for (size_t i = 0; i < physical_devices.size(); i++) {
        ...
        std::cerr << " `--vkdevice " << i << "` ... " << physical_device_properties.deviceName << std::endl;
      }
...
//    vk::PhysicalDeviceFeatures features = physical_device.getFeatures();
    ...
    while (queue_family_index != queue_families.size()) {
      vk::QueueFamilyProperties& queue_family = queue_families.at(queue_family_index);
    ...
    device_create_info.setEnabledExtensionCount(
